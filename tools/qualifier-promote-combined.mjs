/**
 * Promotion gate for the combined repair: oq-v2 -- the new output code plus the
 * policy governing when it applies -- against the production incumbent oq-v1.
 *
 * This is the third gate on related material, so why it is not gate shopping has
 * to be stated plainly. Two earlier experiments asked different questions and
 * both answers stand unchanged:
 *
 ...
 *
 * B produced information that did not exist when A was designed -- over-
 * application is a real failure mode, and the knowledge is what suppresses it.
 * That makes "code plus knowledge against the incumbent" a comparison neither
 * experiment ran, and it is the production question, which neither A nor B was.
 *
 * Scored under spec v3, which narrows CF-FRAUD-MISSED to counterparty fraud.
 * That correction was adjudicated on its own evidence before this gate ran: the
 * rule fired on a correct refusal, and across six versions every stored output on
 * that case declined with the right code while only the ancillary fraud_risk
 * field varied. Rescoring every stored result under v3 changes no arm's mean at
 * all and rescues no earlier decision, which is recorded in the checkpoint.
 *
 ...
 * aggregate it was never aimed at. Both arms run fresh.
 *
 ...
 *
 * Both arms are scored under spec v3 so the scoring model is identical. The
...
export const COMBINED_PROMOTION_CRITERIA = {
  kind: "structural_repair_plus_governing_policy",
...
function responderFor(versionId, model) {
  ...
  return async (record) => {
    ...
    const u = c.usage || {};
    // Scored under spec v3 for both arms. Its codes are a superset of v1's, so
...
const ARMS = [{ id: QUALIFIER_V1_ID, arm: "incumbent" }, { id: QUALIFIER_V2_ID, arm: "candidate" }];
const runs = {};
let usd = 0;
for (const a of ARMS) {
  ...
  for (let t = 0; t < trials; t += 1) {
    const s = await runWorkerEval({
      ...
      note: "combined gate trial " + t + " for " + a.id + " scored under spec v3",
    });
...
const EXPIRY = COMBINED_PROMOTION_CRITERIA.expirySealedCaseIds;
const LIVE = COMBINED_PROMOTION_CRITERIA.liveDeadlineCaseIds;
...
const cand = runs[QUALIFIER_V2_ID];
...
const regressions = Object.keys(incDims)
  ...
  .filter((d) => d.delta < -COMBINED_PROMOTION_CRITERIA.maxDimensionRegression);
...
const candReal = realRecordFixed(QUALIFIER_V2_ID);
const incReal = realRecordFixed(QUALIFIER_V1_ID);

const checks = [
  { id: "non_expiry_non_inferior", pass: candNonExpiry >= incNonExpiry - COMBINED_PROMOTION_CRITERIA.maxNonExpiryRegression,
    detail: "candidate " + candNonExpiry.toFixed(2) + " vs incumbent " + incNonExpiry.toFixed(2) + " (tolerance " + COMBINED_PROMOTION_CRITERIA.maxNonExpiryRegression + ")" },
  ...
  { id: "no_false_expiry_declarations", pass: falseExp.length <= COMBINED_PROMOTION_CRITERIA.maxFalseExpiryDeclarations, detail: falseExp.length ? falseExp.join(", ") : "none" },
  { id: "live_deadline_not_regressed", pass: candLive >= incLive - COMBINED_PROMOTION_CRITERIA.maxLiveDeadlineRegression,
    detail: "candidate " + candLive.toFixed(2) + " vs incumbent " + incLive.toFixed(2) },
  { id: "no_new_critical_failures", pass: newCritical.length <= COMBINED_PROMOTION_CRITERIA.maxNewCriticalFailures, detail: newCritical.length + " introduced" },
  { id: "no_dimension_regression", pass: regressions.length === 0, detail: regressions.length ? regressions.map((r) => r.dimension + " " + r.delta.toFixed(1)).join(", ") : "none beyond " + COMBINED_PROMOTION_CRITERIA.maxDimensionRegression },
...
const decision = {
  ...
  candidate: QUALIFIER_V2_ID, incumbent: QUALIFIER_V1_ID,
  sealedSet: "hemmer-opportunity-qualifier-sealed-v1", sealedSetHash: sealed.sha256, sealedCaseCount: sealed.cases.length,
  scoredUnderSpec: "v3", specV1HashAtRun: specV1HashNow, trials,
  criteria: COMBINED_PROMOTION_CRITERIA,
...
writeFileSync(join(stateDir(), "qualifier-promotion-combined.json"), JSON.stringify(decision, null, 2) + "\n", "utf8");

if (promote) {
  store.putDecision({
    id: "PROMO-OQ-V2-COMBINED", evalRunId: cand[0].runId, agentId: "opportunity_qualifier",
    fromVersionId: QUALIFIER_V1_ID, toVersionId: QUALIFIER_V2_ID, kind: "promotion",
    decidedAt: decision.at, sealedSetHash: sealed.sha256, trials, criteria: COMBINED_PROMOTION_CRITERIA, checks,
...
console.log("\n=== COMBINED PROMOTION: oq-v2 vs oq-v1 ===");
