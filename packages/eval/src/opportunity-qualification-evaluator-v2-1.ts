...
export function planSecondaryReview(input: any) {
  ...
  const plan = [...immediate, ...uncertain.filter((x) => material || blockingCases.has(`${x.opaque_contestant_id}|${x.case_id}`))]; const byCase = new Map<string, any>(); for (const item of plan) { const key = `${item.opaque_contestant_id}|${item.case_id}`; const prior = byCase.get(key) || { ...item, affected_dimension_ids: [] as string[], triggering_frozen_boundaries: [] as string[] }; if (item.dimension_id) prior.affected_dimension_ids.push(item.dimension_id); prior.triggering_frozen_boundaries = item.blocking ? [item.signals[0]] : analysis.triggered; byCase.set(key, prior); }
