...
if(process.argv.length!==4)throw Error('Usage: node packages/foundry/tools/typecheck-pilot.mjs <installed-typescript.js> <installed-@types-directory>');
const ts=require(resolve(process.argv[2]));
const files=readdirSync('packages/foundry/src/pilot').filter(f=>f.endsWith('.ts')).map(f=>'packages/foundry/src/pilot/'+f);
...
console.log(JSON.stringify({compiler:ts.version,semantic:true,strict:true,errors:diagnostics.length,sourceFiles:program.getSourceFiles().filter(f=>!f.isDeclarationFile).length}));process.exitCode=diagnostics.length?1:0;
