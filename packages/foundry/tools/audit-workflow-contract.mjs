...
assert.equal(c.version, 'workflow-029-recovery-r1');
const oldFetch = globalThis.fetch;
globalThis.fetch = () => { throw Error('OFFLINE_AUDIT_NETWORK_DENIED'); };
try {
    prepare(mockRoot, 'mock');
    let state = await runWorkflow(mockRoot, 'W-001-single');
    assert.equal(state.checkpoint, 'waiting_approval');
    const view = approvalView(mockRoot, 'W-001-single');
    approve(mockRoot, 'W-001-single', view.proposalHash, 'fixture-demo');
    state = await runWorkflow(mockRoot, 'W-001-single');
    assert.equal(state.checkpoint, 'completed');
    const result = report(mockRoot);
    assert.equal(result.observations[0].deterministicAccepted, true);
