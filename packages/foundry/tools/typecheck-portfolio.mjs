...
if(process.argv.length!==4)throw Error('Usage: node packages/foundry/tools/typecheck-portfolio.mjs <installed-typescript.js> <installed-@types-directory>');
const ts=require(resolve(process.argv[2]));
const files=readdirSync('packages/foundry/src/portfolio').filter(f=>f.endsWith('.ts')).map(f=>'packages/foundry/src/portfolio/'+f);
...
console.log(JSON.stringify({compiler:ts.version,semantic:true,strict:true,skipDependencyDeclarationCheck:true,errors:diagnostics.length,sourceFiles:program.getSourceFiles().filter(f=>!f.isDeclarationFile).length}));process.exitCode=diagnostics.length?1:0;
