...
export function harness(world = 'viable', options: any = {}) {
    ...
    const tokens: any = {};
    for (const [id, permissions] of [['worker', ['read', 'operate']], ['owner', ['read', 'operate', 'approve', 'promote', 'admin']], ['evaluator', ['read', 'evaluate']]] as [
        string,
        string[]
    ][]) {
        tokens[id] = authority.enroll(id, tenant, business, permissions);
        principals[id] = authority.authenticate(id, tokens[id]);
    }
