...
function page(){
 ...
 for(const id of ['accepted','critical','unnecessary']){const e=new Element('select');e.id=id;}
 ...
 const score=()=>{for(const key of ['correctness','evidenceSupport','uncertainty','escalation','prohibitedPromises'])document.getElementById('dim-'+key).value='true';document.getElementById('accepted').value='true';document.getElementById('critical').value='false';document.getElementById('unnecessary').value='false';document.getElementById('reason').value='Synthetic evidence-based review for test only.';};
