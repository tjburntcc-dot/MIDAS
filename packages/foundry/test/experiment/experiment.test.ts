...
test('mocked development, signed human-review mechanics, procedure-only challenger, validation and freeze integrate', async () => {
    ...
    baseline.development = { method: 'OFFLINE TEST ONLY', measuredHumanSeconds: 0 };
    ...
    assert.equal(frozen.baseline.procedure, baseline.procedure);
});
