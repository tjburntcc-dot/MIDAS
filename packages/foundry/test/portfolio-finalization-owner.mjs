...
const mainRoot=join(repository,'var','finalization-offline-positive-v2'),output=join(repository,'var','finalization-browser');
mkdirSync(output,{recursive:true});
const isolatedRoot=mkdtempSync(join(repository,'var','finalization-owner-managed-'));
...
const completed=portfolio.snapshot().tasks.filter(t=>t.id.endsWith('-v4-r2')&&t.status==='completed');assert.equal(completed.length,5);
...
const binding='quote-desk/review-product-v4-r2';let app,previews,origin,sessionId;
...
try{
 ...
 await owner.screenshot({path:join(output,'isolated-owner-after-restart.png'),fullPage:true});await owner.setViewportSize({width:390,height:844});await owner.goto(origin);await owner.getByRole('button',{name:'Products',exact:true}).click();await owner.waitForTimeout(200);assert.equal(await owner.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),true);await owner.screenshot({path:join(output,'owner-products-mobile.png'),fullPage:true});assert.deepEqual(errors,[]);assert.deepEqual(badResponses,[]);
 console.log(JSON.stringify({status:'passed',isolatedRoot,scope:'Read-only VACUUM INTO snapshot of main state; owner image/toolbar interactions only. Managed DOM provided rendered bounds, never state mutation.',csvContainsCompleted:csv.includes('"completed","20.00"'),offlineCompletedStages:completed.length,mainMutations:0},null,2));
