...
export class PilotIntelligence{
 ...
 /** A deterministic retrieval selection, deliberately separate from commercial
  * diagnosis. It ranks currently permitted retained sources by lexical overlap
  * with owner-supplied company context, never invents an industry answer, and
  * leaves every original source intact for owner inspection and later curation. */
 sources(id:string,discovery:any=null){
  const company=this.knowledge.company(id),all=this.knowledge.selectedSources(id).filter(s=>s.permission==='worker'&&(!s.validUntil||Date.parse(s.validUntil)>Date.now())),terms=[company.name,company.goal,company.notes].join(' ').toLowerCase().match(/[a-z0-9]{3,}/g)??[],queryTerms=[...new Set(terms)].slice(0,48),focusedId=discovery?.focusedSource?.knowledgeSourceId??discovery?.sources?.find((s:any)=>s.id===discovery?.focusedSource?.id)?.knowledgeSourceId??discovery?.focusedSource?.id??null;
  const ranked=all.map(source=>{const haystack=(source.title+' '+source.text.slice(0,24000)).toLowerCase(),score=queryTerms.reduce((total,term)=>total+(haystack.includes(term)?1:0),0);return {source,score};}).sort((a,b)=>b.score-a.score||a.source.title.localeCompare(b.source.title)||a.source.id.localeCompare(b.source.id));
  let selected=ranked.slice(0,32);const focused=ranked.find(item=>item.source.id===focusedId);if(focused&&!selected.some(item=>item.source.id===focused.source.id))selected=[...selected.slice(0,31),focused].sort((a,b)=>b.score-a.score||a.source.title.localeCompare(b.source.title)||a.source.id.localeCompare(b.source.id));
  const focus=selected.find(item=>item.source.id===focusedId)?.source??null,focusedBudget=focus?Math.min(20000,Buffer.byteLength(focus.text)):0,maxPer=Math.floor((32000-focusedBudget)/Math.max(1,selected.length-(focus?1:0)));
  return selected.map((item,index)=>{const s=item.source,budget=s.id===focusedId?focusedBudget:maxPer;let text=s.text;while(Buffer.byteLength(text)>budget)text=text.slice(0,Math.max(0,text.length-100));return {...s,text,retrieval:{rank:index+1,score:item.score,method:'mechanical lexical overlap with owner-supplied company context and source title; not a business diagnosis',focused:s.id===focusedId},excerpt:{offset:0,characters:text.length,totalCharacters:s.text.length,complete:text.length===s.text.length,reason:s.id===focusedId?'Worker-selected permitted source given additional context capacity.':'Mechanically ranked bounded retrieval excerpt; full retained document remains available to owner inspection and explicit source selection.'}};});
 }
 ...
 executionSources(id:string,requiredIds:string[],limits={maxSources:24,maxSourceCharacters:24000,maxBytes:160000}){
  requireThat(Number.isSafeInteger(limits.maxSources)&&limits.maxSources>0&&limits.maxSources<=24&&Number.isSafeInteger(limits.maxSourceCharacters)&&limits.maxSourceCharacters>0&&limits.maxSourceCharacters<=24000&&Number.isSafeInteger(limits.maxBytes)&&limits.maxBytes>0&&limits.maxBytes<=160000,'INTELLIGENCE_EXECUTION_SOURCE_LIMITS');
  ...
  const chosen:any[]=[];let bytes=0;for(const s of [...required,...remaining]){const fit=chosen.length<limits.maxSources&&s.text.length<=limits.maxSourceCharacters&&bytes+Buffer.byteLength(s.text)<=limits.maxBytes;if(!fit){requireThat(!requiredIds.includes(s.id),'INTELLIGENCE_REQUIRED_EVIDENCE_NEEDS_BOUNDED_EXCERPT');continue;}chosen.push(s);bytes+=Buffer.byteLength(s.text);}return chosen;
 }
