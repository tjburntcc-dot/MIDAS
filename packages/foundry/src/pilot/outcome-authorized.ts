import {adaptiveContract,adaptiveEnabled,adaptiveProcedure,validateAdaptiveWorker,ADAPTIVE_TOOL,ADAPTIVE_VERSION} from '../adaptive/worker-contract.ts';
...
import {OUTCOME_PLANNER_PROCEDURE,outcomePlannerProcedure,outcomeFamilies,outcomePlanSchema,validateOutcomePlan} from './outcome-contract.ts';
...
function stableMandate(o:any){return {id:o.id,businessId:o.businessId,objective:o.objective,autonomy:o.autonomy,allowedFamilies:o.allowedFamilies,maxCalls:o.maxCalls,repairReserve:o.repairReserve,effects:o.effects,createdAt:o.createdAt,...(o.adaptive?{adaptive:o.adaptive}:{})};}
...
function sourcePaths(directory=join(PACKAGE_ROOT,'src'),base=PACKAGE_ROOT):string[]{
 ...
 return readdirSync(directory,{withFileTypes:true}).flatMap(entry=>entry.isDirectory()?sourcePaths(join(directory,entry.name),base):entry.isFile()&&/\.(ts|py)$/.test(entry.name)?[cleanRelative(relative(base,join(directory,entry.name)))]:[]).sort();
}
...
function profileFor(services:Services,businessId:string,family:OutcomeFamily,adaptive?:AdaptiveOptions):Profile{
 const assignment=new PilotLearning(services.outcomes.store).assignment(businessId,family),compiled=adaptive?adaptiveContract(adaptive):null;
 return {family,capability:assignment.capability,procedureHash:compiled?hash(compiled.procedure):assignment.procedureHash,schemaHash:hash(compiled?.schema??workerSchema),allowedTools:[...LOCAL_TOOLS,...(compiled?[ADAPTIVE_TOOL]:[])],effect:'local_preparation_only',minimumCalls:family==='functional-project'?7:3,executionProfile:family==='functional-project'?'functional-project-v1':family==='marketing-page'?'marketing-page-v1':family==='business-site'?'business-site-v1':family==='campaign-packet'?'commercial-campaign-v1':null};
}
...
function validateGrant(services:Services,root:string,g:OutcomeGrant,approved:boolean){
 ...
 for(const p of b.profiles){const expected=profileFor(services,b.businessId,p.family,services.outcomes.get(b.outcomeId).adaptive);requireThat(b.allowedFamilies.includes(p.family)&&hash(p)===hash(expected)&&p.allowedTools.every(t=>[...ALL_TOOL_NAMES,...(services.outcomes.get(b.outcomeId).adaptive?[ADAPTIVE_TOOL]:[])].includes(t as any)),'OUTCOME_PROFILE_CHANGED');}
 ...
 requireThat(b.planner.requestHash===hash(b.planner.request)&&b.planner.procedureHash===hash(outcomePlannerProcedure(services.outcomes.get(b.outcomeId).adaptive))&&b.planner.schemaHash===hash(outcomePlanSchema)&&b.planner.bodyHash===bodyHash(g,b.planner.request,outcomePlanSchema),'OUTCOME_PLANNER_BINDING_CHANGED');
...
function effectiveTaskProcedureHash(store:any,task:any){
 if(adaptiveEnabled(task))return hash(adaptiveProcedure(task));
...
export function loadAuthorizedOutcome(services:Services,root:string,options:{}|{testing:{envelope:any;trustedPublicKey:string;transport:typeof fetch}}={}){
 ...
 const assertAdaptiveBinding=(task:any)=>{const expected=services.outcomes.get(b.outcomeId).adaptive;requireThat(hash((task.inputs as any)?.adaptiveExecution??null)===hash(expected?{version:ADAPTIVE_VERSION,...expected}:null),'OUTCOME_TASK_ADAPTIVE_SCOPE_CHANGED');};
 const bindTask=async(taskId:string)=>{
  current();let {o,node,task}=taskNode(services,g,taskId);assertAdaptiveBinding(task);requireThat(task.ventureId===b.businessId&&task.status==='queued'&&!task.lease,'OUTCOME_UNSTARTED_TASK_REQUIRED');const profile=b.profiles.find(p=>p.family===node.family),correction=node.generation>0&&Boolean((task.inputs as any)?.correctionOf),expectedCapability=correction?'quality.review':profile?.capability,minimum=correction?3:profile?.minimumCalls;requireThat(profile&&task.capability===expectedCapability&&task.effectAuthority?.kind==='local'&&task.effectAuthority.reference==='pilot-032-local-preparation-only'&&task.allowedTools.every((t:string)=>profile.allowedTools.includes(t))&&task.resource.modelCalls===node.calls&&node.calls>=minimum!,'OUTCOME_TASK_PROFILE_DENIED');
  ...
  requireThat(task.attempts===0,'OUTCOME_UNSTARTED_TASK_REQUIRED');
  ...
  const preview=await engine.previewRequest(taskId),procedureHash=hash(preview.request.role.procedure),expectedProcedureHash=adaptiveEnabled(task)?profile.procedureHash:correction?b.correctionProcedureHash:effectiveTaskProcedureHash(store,task);requireThat(procedureHash===expectedProcedureHash&&hash(preview.schema)===profile.schemaHash,'OUTCOME_TASK_PROCEDURE_OR_SCHEMA_CHANGED');
 ...
 const workerInvoke=async(call:WorkerCall,recover:boolean)=>{
  const task=rows(store,'portfolio-task').find((t:any)=>t.ventureId===b.businessId&&scopeKey(workerScope(t.ventureId,t.id))===scopeKey(call.request.scope));requireThat(task,'OUTCOME_WORKER_TASK_MISSING');assertAdaptiveBinding(task);const binding=store.get('pilot-authorized-outcome-task',bindingPrefix+task.id);requireThat(binding?.grantHash===grantHash,'OUTCOME_WORKER_TASK_NOT_BOUND');
  ...
  if(!recover){current();services.outcomes.execution.assertContextCurrent(task.id,true);}const result=await invoke(binding.stage,call.request,call.schema,adaptiveEnabled(task)?out=>validateAdaptiveWorker(task,out):validateWorker,recover);services.outcomes.execution.assertContextCurrent(task.id,true);return result;
 };
 ...
 engine=new PortfolioEngine({portfolio:services.outcomes.execution.portfolio,tools:services.outcomes.execution.workTools(worker.kind==='actual_model'?'actual_model':'fixture'),evidence:services.outcomes.execution.evidence,prepareTask:services.outcomes.execution.engine.prepareTask,model:worker,accounting:totals,validateTask:task=>{current();services.outcomes.execution.assertContextCurrent(task.id,true);}});
 const runTask=async(taskId:string)=>{await engine.recover();let task=nodeTask(services,taskId);if(task.status==='needs_reconciliation'){requireThat(store.get('pilot-authorized-outcome-task',bindingPrefix+taskId),'OUTCOME_RECOVERY_TASK_NOT_BOUND');const execution=store.get('portfolio-execution',taskId),step=execution&&store.get('portfolio-step',taskId+'/model-'+execution.index);if(!step||!['reserved','uncertain'].includes(step.status))return task;await engine.resumeResponse(taskId);task=nodeTask(services,taskId);}if(task.status==='completed')return task;await bindTask(taskId);return engine.runTask(taskId,(nodeTask(services,taskId).inputs as any).assignedWorkerId??'pilot-generalist');};
 const run=async()=>{current();await engine.recover();requireThat(services.outcomes.get(b.outcomeId).plan?.decision==='prepare','OUTCOME_PLAN_REQUIRED');for(let pass=0;pass<b.maximumTasks+2;pass++){services.outcomes.materialize(b.outcomeId);const o=services.outcomes.get(b.outcomeId);let advanced=false;for(const node of o.graph){if(!node.taskId)continue;const task=nodeTask(services,node.taskId);if(['queued','needs_reconciliation'].includes(task.status)){await runTask(task.id);advanced=true;}}if(!advanced)break;}return services.outcomes.refresh(b.outcomeId);};
