...
export function report(store:StateStore,principal:Principal,s:Scope) {
  assertScope(principal,s);const run=store.get('run',scopeKey(s));requireThat(run,'RUN_NOT_FOUND');
  ...
  const costEntries=events.filter(e=>e.cost || e.usage?.cost).map(e=>({event:e.kind,at:e.at,category:e.kind.startsWith('model')?'model':e.kind==='approval_granted'?'human_review':e.kind.startsWith('evidence')?'tool_read':'tool_action',cost:e.cost ?? e.usage.cost,simulated:e.simulated ?? true}));
...
export function exportReport(store:StateStore,principal:Principal,s:Scope,directory:string) {
  const value=report(store,principal,s);mkdirSync(directory,{recursive:true});
  const raw=JSON.stringify(value,null,2)+'\n';const path=join(directory,'report.json');writeFileSync(path,raw);
  if(value.artifact)writeFileSync(join(directory,'artifact.json'),JSON.stringify(value.artifact,null,2)+'\n');
  const normalized=normalizedReport(value);writeFileSync(join(directory,'normalized.json'),canonical(normalized)+'\n');
