...
export type Roles = { analyst: Role; operator: Role; verifier: Role };
...
export type EvaluationReport = {
  ...
  evaluationManifest: {
    ...
    scoringHash: string;
    accessPolicy: { learner: 'development_manifest_only'; evaluator: 'fixture_case_only'; approver: 'report_only' };
    input: { requireDelivery: boolean; requireVerification: boolean; maximumSteps: number | null };
  };
  replay: {
    predicate: string;
    incumbent: { procedure: string; observation: Record<string, unknown>; passed: boolean };
    candidate: { procedure: string; observation: Record<string, unknown>; passed: boolean };
  };
  thresholds: { candidatePasses: boolean; incumbentPasses: boolean; improvement: number; met: boolean };
...
type Registry = {
  schemaVersion: '1';
  business: string;
  roles: { [K in keyof Roles]: { active: Role; history: Record<string, Role> } };
};

type RunPin = { schemaVersion: '1'; scope: string; roles: Roles; registryVersion: number };
...
function observedFailure(outcome: Record<string, unknown>): Record<string, unknown> | null {
  ...
  const resultStatus = result !== null && typeof result === 'object' && !Array.isArray(result)
    ...
    : outcome.status;
  if (resultStatus === 'passed' || resultStatus === 'pass' || resultStatus === 'confirmed') return null;
  const supplied = outcome.failure;
  if (supplied === null || supplied === undefined || supplied === false) return null;
  if (typeof supplied === 'object' && !Array.isArray(supplied)) return clone(supplied as Record<string, unknown>);
  return { supplied };
}
...
function replayProcedure(procedure: string, input: { requireDelivery: boolean; requireVerification: boolean; maximumSteps: number | null }): Record<string, unknown> {
...
function predicate(observation: Record<string, unknown>, input: { requireDelivery: boolean; requireVerification: boolean; maximumSteps: number | null }): boolean {
...
export class LearningService {
  ...
  initialize(s: Scope): Roles {
    ...
    return this.store.transaction(() => {
      let registry = this.store.get(REGISTRY, business) as (Registry & { _version: number }) | null;
      if (registry === null) {
        registry = this.store.put(REGISTRY, business, registryFrom(initialRoles(), business), null) as Registry & { _version: number };
      }
      const existingPin = this.store.get(RUN_PIN, run) as (RunPin & { _version: number }) | null;
      if (existingPin === null) {
  ...
  roles(s: Scope): Roles {
    const pin = this.store.get(RUN_PIN, scopeKey(s)) as (RunPin & { _version: number }) | null;
  ...
  read(s: Scope, principal: Principal, candidateId: string): LearningRead {
    ...
    const evaluation = this.store.get(EVALUATION, candidateKey(s, candidateId)) as (EvaluationReport & { _version: number }) | null;
    if (evaluation !== null) this.verifyEvaluationRecord(s, principal, candidate, evaluation);
    const decision = this.store.get(DECISION, candidateKey(s, candidateId)) as (PromotionDecision & { _version: number }) | null;
    if (decision !== null) this.verifyDecisionRecord(s, principal, candidate, evaluation, decision);
  ...
  propose(s: Scope, principal: Principal, outcome: unknown): LearningCandidate {
    ...
    return this.store.transaction(() => {
      ...
      const origin = this.store.get(CANDIDATE_ORIGIN, originKey) as { candidateId: string; originHash: string; _version: number } | null;
      if (origin !== null) {
        requireThat(origin.originHash === originHash, 'CANDIDATE_ORIGIN_CORRUPT');
        return clone(this.readCandidate(s, origin.candidateId));
      }
      const sequence = this.store.get(CANDIDATE_SEQUENCE, run) as { next: number; _version: number } | null;
      const next = sequence === null ? 1 : sequence.next + 1;
      if (sequence === null) this.store.put(CANDIDATE_SEQUENCE, run, { next }, null);
      else this.store.put(CANDIDATE_SEQUENCE, run, { next }, sequence._version);

      const candidateId = `candidate-${hash(run).slice(0, 16)}-${next}`;
      identifier(candidateId);
      const failureRef = this.store.record(
        s, `failure-${candidateId}`, 'fixture_failure',
        (outcome.failure as unknown) ?? { source: 'synthetic_fixture', outcomeHash: hash(outcome) },
      );
  ...
  evaluate(s: Scope, evaluator: Principal, candidateId: string, fixtureCase: FixtureCase = 'inconclusive'): EvaluationReport {
    ...
    return this.store.transaction(() => {
      ...
      const existing = this.store.get(EVALUATION, candidateKey(s, candidateId)) as (EvaluationReport & { _version: number }) | null;
      if (existing !== null) {
  ...
  decide(s: Scope, approver: Principal, candidateId: string): PromotionDecision {
    ...
    return this.store.transaction(() => {
      ...
      const existing = this.store.get(DECISION, candidateKey(s, candidateId)) as (PromotionDecision & { _version: number }) | null;
      if (existing !== null) {
      ...
      if (evaluation.verdict === 'passing' && evaluation.thresholds.met) {
        ...
        const registry = this.store.get(REGISTRY, key) as Registry & { _version: number };
        ...
        const registry = this.store.get(REGISTRY, businessKey(s)) as Registry & { _version: number };
  ...
  private readCandidate(s: Scope, candidateId: string): LearningCandidate {
    const candidate = this.store.get(CANDIDATE, candidateKey(s, candidateId)) as (LearningCandidate & { _version: number }) | null;
  ...
  private verifyCandidateRecord(s: Scope, principal: Principal, candidate: LearningCandidate): void {
    ...
    const { recordRef, _version, ...candidateWithoutRecord } = candidate as LearningCandidate & { _version?: number };
  ...
  private verifyEvaluationRecord(s: Scope, principal: Principal, candidate: LearningCandidate, evaluation: EvaluationReport): void {
    ...
    const { recordRef, _version, ...evaluationWithoutRecord } = evaluation as EvaluationReport & { _version?: number };
  ...
  private verifyDecisionRecord(s: Scope, principal: Principal, candidate: LearningCandidate, evaluation: EvaluationReport | null, decision: PromotionDecision): void {
    ...
    const { recordRef, _version, ...decisionWithoutRecord } = decision as PromotionDecision & { _version?: number };
