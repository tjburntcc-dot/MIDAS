...
export class StateStore {
  ...
  transaction<T>(fn: () => T): T {
    ...
    try { const result = fn(); requireThat(!(result instanceof Promise), 'ASYNC_TRANSACTION_FORBIDDEN'); this.db.exec('COMMIT'); return result; }
  ...
  put(kind: string, key: string, value: any, expectedVersion: number | null): any {
    const { _version, ...body } = value; const version = expectedVersion === null ? 1 : expectedVersion + 1;
    if (expectedVersion === null) this.db.prepare('INSERT INTO entities(kind,key,version,body) VALUES(?,?,?,?)').run(kind,key,version,canonical(body));
    else requireThat(this.db.prepare('UPDATE entities SET version=?,body=? WHERE kind=? AND key=? AND version=?').run(version,canonical(body),kind,key,expectedVersion).changes === 1, 'STALE_AGGREGATE');
  ...
  events(principal: Principal, s: Scope): any[] {
    assertScope(principal,s); return this.db.prepare('SELECT seq,kind,body FROM events WHERE scope=? ORDER BY seq').all(scopeKey(s)).map(r => ({ seq:r.seq,kind:r.kind,...JSON.parse(String(r.body)) }));
  }
  record(s: Scope, id: string, kind: string, value: any, parents: Ref[] = []): Ref {
    ...
    if(existing) { const prior=JSON.parse(String(existing.body)); requireThat(hash({kind,value,parents})===hash({kind:prior.kind,value:prior.value,parents:prior.parentRefs}),'RECORD_IMMUTABLE'); return {id,version:String(existing.version),sha256:String(existing.sha256)}; }
    const body = { id, kind, schemaVersion:'1', scope:s, createdAt:new Date().toISOString(), parentRefs:parents, value };
  ...
  records(principal: Principal, s: Scope): any[] {
    assertScope(principal,s); return this.db.prepare('SELECT body,sha256 FROM records WHERE scope=? ORDER BY rowid').all(scopeKey(s)).map(r => ({...JSON.parse(String(r.body)),sha256:r.sha256}));
  }
  artifact(s: Scope, id: string, body: any): Ref {
    const bytes = canonical(body); const digest = hash(body);
    const previous = this.db.prepare('SELECT sha256 FROM artifacts WHERE scope=? AND id=?').get(scopeKey(s),id);
    if (previous) requireThat(previous.sha256 === digest, 'ARTIFACT_IMMUTABLE');
    else this.db.prepare('INSERT INTO artifacts(scope,id,sha256,body) VALUES(?,?,?,?)').run(scopeKey(s),id,digest,bytes);
    return { id, version:'1', sha256:digest };
  }
  readArtifact(principal: Principal, s: Scope, id: string): any {
    assertScope(principal,s); const row = this.db.prepare('SELECT body,sha256 FROM artifacts WHERE scope=? AND id=?').get(scopeKey(s),id);
    requireThat(row, 'ARTIFACT_NOT_FOUND'); const body=JSON.parse(String(row.body)); requireThat(hash(body)===row.sha256,'ARTIFACT_CORRUPT'); return body;
  }
}
