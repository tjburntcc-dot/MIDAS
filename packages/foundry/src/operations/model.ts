...
export type OperatingGrant = {
    ...
    businesses:Array<{id:string; goalHash:string; sourceHosts:string[]; maxCalls:number}>;
    retries:0; providerConcurrency:1; countDeadlineMs:10000; externalAuthority:false;
};
export function implementationHash() {
    const here=dirname(fileURLToPath(import.meta.url));
    return hash(['model.ts','contracts.ts','manager.ts','research.ts','mail.ts','learning.ts','communication.ts','study.ts','trusted.ts','server.ts','cli.ts','../model-port.ts','../state.ts','../experiment/ledger.ts','../experiment/token-count.ts','../experiment/config.ts'].map(p=>readFileSync(join(here,p),'utf8').replace(/\r\n/g,'\n')));
}
export type Invocation = {businessId:string;goalHash:string;sourceHosts:string[];attemptId:string;stage:string;request:ModelRequest;schema:any;validate:(out:any)=>void};
export class OperatingModels {
    ...
    constructor(options:{store:StateStore;root:string;envelope:any;trustedPublicKey:string;execution:{kind:'mock';transport:typeof fetch}|{kind:'live';credential:()=>string;transport:typeof fetch}}) {
        ...
        for(const b of g.businesses){identifier(b.id);safeInteger(b.maxCalls,1);requireThat(/^[a-f0-9]{64}$/.test(b.goalHash),'OPERATING_GOAL_HASH');}
    ...
    async invoke(principal:Principal,x:Invocation):Promise<ModelResult> {
        ...
        const budget=this.ledger.port('development',{businessId:b.id,stage:x.stage,source:g.mode==='mock'?'offline_mock':'actual-model',goalHash:b.goalHash});
