...
type StoredRun = { frozenRef: Ref; attempts: ProcedureAttempt[]; blockedReason?:string; _version: number };
...
export class ProcedureLab {
    ...
    async compare(principal: Principal, scope: Scope, frozenId: string, invoker: OperatingModelInvoker): Promise<{ frozen: FrozenProcedureComparison; attempts: ProcedureAttempt[]; analysis: ProcedureAnalysis }> {
        ...
        comparisonLoop: for (let caseIndex = 0; !run.blockedReason && caseIndex < frozen.cases.length; caseIndex++) { const testCase = frozen.cases[caseIndex], order = caseIndex % 2 === 0 ? ['baseline', 'challenger'] as const : ['challenger', 'baseline'] as const; for (const condition of order) {
            ...
            let result: { output: unknown; usage?: unknown; route?: unknown }; try { result = await invoker.invoke(clone(attempt.request)); } catch (error) { const code=errorCode(error);this.finish(scope, runKey, frozen, attempt, { status: 'failed', errorCode: code });if(/HTTP|ACCESS|AUTHORITY|SENSITIVE|UNCERTAIN|GRANT|BUDGET|CAP|HALTED|MISMATCH/.test(code)){this.store.transaction(()=>{const current=this.store.get(RUN,runKey) as StoredRun;this.store.put(RUN,runKey,{...current,blockedReason:code},current._version);});break comparisonLoop;}sequenceIndex++; continue; }
