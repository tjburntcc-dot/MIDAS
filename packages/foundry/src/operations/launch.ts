...
export function prepareGmailTest(root:string,m:OperatingManager,sender:string,recipients:string[]){
 ...
 requireThat(email.test(sender)&&recipients.length===2&&recipients.every(r=>email.test(r)&&r.toLowerCase()!==sender.toLowerCase())&&new Set(recipients.map(r=>r.toLowerCase())).size===2,'TWO_DISTINCT_CONTROLLED_RECIPIENTS_REQUIRED');
