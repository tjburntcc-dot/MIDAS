...
export type FixtureModelResponse = {
    output: InformationRequest | DecisionProposal | OperatorOutput | {
...
export type FixtureModel = {
    ...
    run(request: ModelRequest): Promise<FixtureModelResponse>;
};
    ...
    return {
        ...
        async run(request) {
            ...
            if (request.task === "investigate")
                ...
                output = hasEvidence(request) ? decision(tape) : informationRequest();
            else if (request.task === "operate")
                output = operatorOutput(tape);
            else
                output = { kind: "verification_note", text: "Fixture output is not verification; the laboratory evaluator requires a receipt and independent observation." };
            return {
                output,
