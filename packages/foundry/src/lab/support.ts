...
export type SupportSnapshot = {
    ...
    claims: Array<{
        id: string;
        proposition: string;
        evidenceClass: "reported" | "hypothetical" | "contradicted";
    ...
    rights: Array<{
        toolId: string;
        effectClass: "read" | "reversible_write";
...
export type SupportEnvironment = {
    ...
    validateDecision(snapshot: SupportSnapshot, evidence: EvidenceResponse[], decision: DecisionProposal): {
        ok: boolean;
        reason: string;
    };
...
function assertEvidenceRequest(request: EvidenceRequest): void {
    if (!request || typeof request !== "object" || !hasOnlyKeys(request, ["kind", "variable", "decision", "plausibleRange", "branches", "source", "maxCost", "deadline"]))
        throw new Error("Unsupported evidence request fields.");
    if (request.kind !== undefined && request.kind !== "information_request")
        throw new Error("Unsupported evidence request kind.");
    if (request.variable !== "cost_per_case" && request.variable !== "publication_policy")
        throw new Error("Unsupported evidence variable.");
    if (typeof request.decision !== "string" || request.decision.length === 0 || typeof request.deadline !== "string" || Number.isNaN(Date.parse(request.deadline)))
        throw new Error("Evidence request must include a decision and ISO deadline.");
    if (!request.maxCost || !isNonnegativeMinorMoney(request.maxCost))
        throw new Error("Evidence request maxCost must be a nonnegative safe USD minor-unit amount.");
    ...
    if (request.source !== expectedSource)
        throw new Error("Unsupported evidence source for requested variable.");
}
function providedCost(evidence: EvidenceResponse[], id: string): number | undefined {
    for (const result of evidence) {
        if (result.status !== "provided" || result.requested.variable !== "cost_per_case")
            continue;
        const item = result.evidence.find((candidate) => candidate.id === id && candidate.variable === "cost_per_case");
        if (!item || typeof item.value !== "number" || !Number.isSafeInteger(item.value) || item.value < 0 || item.unit !== "USD cents per contacted or assisted customer")
            continue;
        return item.value;
    }
    return undefined;
...
function policyEvidenceState(evidence: EvidenceResponse[]): "provided" | "conflicting" | "missing" {
    ...
    if (results.some((result) => result.status === "conflicting"))
        return "conflicting";
    if (results.some((result) => result.status === "provided"))
        return "provided";
    return "missing";
}
...
function validArtifact(artifact: unknown): artifact is SupportArtifact {
    if (!artifact || typeof artifact !== "object" || !hasOnlyKeys(artifact, ["title", "policyVersion", "steps", "answers"]))
        return false;
...
function validDecisionShape(decision: unknown): decision is DecisionProposal {
    if (!decision || typeof decision !== "object" || !hasOnlyKeys(decision, ["kind", "chosenOptionId", "status", "alternatives", "rationale", "assumptions", "reversalConditions", "experiment"]))
        return false;
...
function validSimulatedLedger(ledger: unknown): ledger is SimulatedLedger {
    if (!ledger || typeof ledger !== "object" || !hasOnlyKeys(ledger, ["simulated", "bookings", "collections", "recognizedRevenue", "refunds", "obligations", "modeledCustomerSavings", "note"]))
        return false;
...
export function createSupportEnvironment(world: SupportWorld = "viable", overrides: SupportEnvironmentOverrides = {}): SupportEnvironment {
    ...
    return {
        ...
        validateDecision(inputSnapshot, evidence, decision) {
            if (inputSnapshot.companyOverlay.simulated !== true)
                return { ok: false, reason: "Laboratory adapter requires a simulated company overlay." };
            if (!validDecisionShape(decision))
                return { ok: false, reason: "Decision has unsupported or malformed nested fields." };
            const policyState = policyEvidenceState(evidence);
            if (policyState === "missing")
                return { ok: false, reason: "A policy decision requires separately retrieved qualified policy evidence." };
            ...
            if (highBenefit === undefined || narrowBenefit === undefined)
                return { ok: false, reason: "Public benefit facts are missing or invalid." };
            if (highCost === undefined || narrowCost === undefined)
                return { ok: decision.status === "blocked" && decision.chosenOptionId === "no-action", reason: decision.status === "blocked" && decision.chosenOptionId === "no-action" ? "Cost remains qualified as unknown and the action remains blocked." : "A contribution decision requires qualified cost evidence." };
            ...
            if (decision.alternatives.length !== 3 || !high || !narrow || !noAction || !matchesAlternative(high, "high-touch-outreach", highBenefit, highCost, ["evidence-cost-high-touch"]) || !matchesAlternative(narrow, "limited-playbook", narrowBenefit, narrowCost, ["evidence-cost-playbook"]) || !matchesAlternative(noAction, "no-action", 0, 0, []))
                return { ok: false, reason: "Decision alternatives do not match public benefits, qualified costs, or evidence references." };
            if (policyConflict)
                return { ok: decision.status === "blocked" && decision.chosenOptionId === "no-action", reason: decision.status === "blocked" && decision.chosenOptionId === "no-action" ? "Conflicting policy facts correctly block delivery." : "Conflicting policy facts block delivery." };
            const best = high.contribution.minorUnits > narrow.contribution.minorUnits ? high : narrow;
            if (best.contribution.minorUnits <= 0)
                return { ok: decision.status === "rejected" && decision.chosenOptionId === "no-action", reason: decision.status === "rejected" && decision.chosenOptionId === "no-action" ? "All interventions have nonpositive contribution, so the evidence-backed no-go is valid." : "All interventions have nonpositive contribution; delivery must be rejected." };
        ...
        actionFor(output) {
            if (!validOperatorOutput(output))
                throw new Error("Operator output does not match the support artifact schema.");
        ...
        verify(input) {
            ...
            if (!validArtifact(artifact))
                failures.push("Missing or malformed support artifact.");
            if (!input.receipt || input.receipt.toolId !== "lab.publish" || input.receipt.status !== "confirmed")
                failures.push("No confirmed laboratory publication receipt.");
            if (!input.observation || input.observation.status !== "confirmed" || input.observation.deliveryObserved !== true)
                failures.push("Durable service observation does not confirm delivery.");
            if (!input.observation?.artifact || !validArtifact(input.observation.artifact) || !artifact || JSON.stringify(input.observation.artifact) !== JSON.stringify(artifact))
                failures.push("Durable service observation does not match the submitted artifact.");
            if (!validSimulatedLedger(input.observation?.ledger))
                failures.push("Durable service observation lacks a valid simulated ledger.");
            if (!policy || !artifact || artifact.policyVersion !== policy.version || input.observation?.artifact?.policyVersion !== policy.version)
                failures.push("Artifact or observation does not match the active publication policy.");
            ...
            if (passed !== verificationTickets.length)
                failures.push("Artifact does not satisfy the evaluator-held synthetic ticket coverage requirement.");
