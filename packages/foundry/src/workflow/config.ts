...
import { recoveryVersion, recoverySchedule, captureParent, recoveryLimits, validateRecovery, claimRecovery, checkRecoveryBinding } from './recovery.ts';
...
export function prepareConfig(root: string, mode: 'mock' | 'live', cases: any, profile: 'original' | 'value' | 'recovery' = 'original', parentRoot?: string) {
    ...
    const link = profile === 'recovery' ? captureParent(parentRoot ?? '', mode) : null;
    ...
    const selectedVersion = profile === 'recovery' ? recoveryVersion : profile === 'value' ? valueVersion : version;
    const config = { version: selectedVersion, ...(link ? { link } : {}), ...(profile === 'value' ? { valuePolicy } : {}), mode, root, implementationHash: implementationHash(), casesHash: hash(cases), schedule: link ? recoverySchedule : profile === 'value' ? valueSchedule() : schedule(), route, limits: link ? recoveryLimits(link) : limits(), preparedAt: new Date().toISOString(), provenance: mode === 'mock' ? 'OFFLINE MOCK — no model competence or external spending' : 'PROSPECTIVE LIVE — separate signed authorization required' };
    write(root, 'config.json', config, true);
    write(root, 'episodes.json', cases, true);
    write(root, 'authorization.request.json', { kind: link ? 'workflow-execution-amendment' : 'workflow-execution-grant', version: selectedVersion, approved: false, root, configHash: hash(config), implementationHash: config.implementationHash, projectId: 'proj_H01ORqdOPQM6vdGwQYsqFL5r', credentialFile: 'C:/Users/14844/Downloads/MIDAS/var/foundry-worktree-028/var/foundry-smoke-028/auth/provider/openai.key', route, limits: config.limits, ...(link ? { link, availability: valuePolicy.availability, countDeadlineMs: 10000, inferenceAdmissions: 4, countRequests: 4, additionalInferenceReservationMinor: 208, combinedInferenceCeiling: 48, combinedCountCeiling: 48, countBufferReused: true, permittedRunId: recoverySchedule[0].runId, furtherRecovery: false } : {}), data: 'synthetic workflow episodes only; no customer/protected data', expiresAt: '2026-09-25T22:00:00Z', reviewer: 'Mason', humanMinutes: profile !== 'original' ? null : 210, ...(profile === 'value' ? { availability: valuePolicy.availability, countDeadlineMs: 10000, inferenceAdmissions: 48, countRequests: 48, firstRelease: valuePolicy.firstRelease, teamAdmitted: false } : {}), recoveryAdmissions: link ? 4 : 0, allowProtected: false, allowBusinessActions: false }, true);
...
export function configFor(root: string) { root = resolve(root); const c = read(root, 'config.json'); requireThat([version, valueVersion, recoveryVersion].includes(c.version) && c.root === root, 'WORKFLOW_ROOT_BINDING'); requireThat(c.implementationHash === implementationHash(), 'IMPLEMENTATION_CHANGED'); requireThat(c.casesHash === hash(read(root, 'episodes.json')), 'CASE_MANIFEST_CHANGED'); requireThat(hash(c.route) === hash(route) && hash(c.limits) === hash(c.version === recoveryVersion ? recoveryLimits(c.link) : limits()) && hash(c.schedule) === hash(c.version === recoveryVersion ? recoverySchedule : c.version === valueVersion ? valueSchedule() : schedule()) && (c.version !== valueVersion || hash(c.valuePolicy) === hash(valuePolicy)), 'CONFIGURATION_CHANGED'); if (c.version === recoveryVersion) validateRecovery(c); return c; }
export function authorize(root: string, approvalFile: string) { const c = configFor(root); requireThat(c.mode === 'live', 'LIVE_GRANT_ROOT_REQUIRED'); const request = read(root, 'authorization.request.json'), approval = JSON.parse(readFileSync(approvalFile, 'utf8')); requireThat(approval.approved === true && hash({ ...approval, approved: false }) === hash(request), 'EXACT_APPROVED_GRANT_REQUIRED'); requireThat(hash(approval.route) === hash(c.route) && hash(approval.limits) === hash(c.limits) && approval.allowProtected === false && approval.allowBusinessActions === false && approval.recoveryAdmissions === (c.version === recoveryVersion ? 4 : 0), 'GRANT_LIMITS_MISMATCH'); requireThat(Date.parse(approval.expiresAt) > Date.now(), 'GRANT_EXPIRED'); if (c.version === recoveryVersion) { requireRecoveryPreflight(root, c); requireThat(approval.projectId === c.link.projectId && approval.credentialFile === c.link.credentialFile && approval.expiresAt === c.link.expiresAt, 'RECOVERY_ACCESS_CHANGED'); claimRecovery(c); } else if (c.version === valueVersion) claimValueAccount(root, c); else requireThat(!existsSync(join(dirname(resolve(root)), 'workflow-029-value-account.json')), 'VALUE_ACCOUNT_ALREADY_BOUND'); const signature = sign(null, Buffer.from(canonical(approval)), readFileSync(join(root, 'auth', 'owner.key'))).toString('base64'); write(root, 'authorization.json', { statement: approval, signature }, true); return { signed: true, grantHash: hash(approval) }; }
export function checkGrant(root: string, c: any) { if (c.mode === 'mock')
    return { hash: hash(c), statement: null }; requireThat(existsSync(join(root, 'authorization.json')), 'LIVE_EXECUTION_NOT_AUTHORIZED'); const e = read(root, 'authorization.json'), s = e.statement; requireThat(verify(null, Buffer.from(canonical(s)), readFileSync(join(root, 'auth', 'owner.pub')), Buffer.from(e.signature, 'base64')), 'GRANT_SIGNATURE_INVALID'); requireThat(hash(s.route) === hash(c.route) && hash(s.limits) === hash(c.limits) && s.allowProtected === false && s.allowBusinessActions === false && s.recoveryAdmissions === (c.version === recoveryVersion ? 4 : 0), 'GRANT_LIMITS_MISMATCH'); requireThat(s.approved === true && s.configHash === hash(c) && s.root === resolve(root) && s.implementationHash === c.implementationHash && hash({ ...s, approved: false }) === hash(read(root, 'authorization.request.json')), 'GRANT_BINDING_MISMATCH'); requireThat(Date.parse(s.expiresAt) > Date.now(), 'GRANT_EXPIRED'); if (c.version === recoveryVersion) { checkRecoveryBinding(c); requireRecoveryPreflight(root, c); } return { hash: hash(s), statement: s }; }
...
function requireRecoveryPreflight(root: string, c: any) { const p = read(root, 'contract-preflight.json'); requireThat(p.implementationHash === c.implementationHash && p.configHash === hash(c) && p.passed === true && p.providerRequests === 0 && hash(p.stages) === hash(['investigate','decide','operate','verify']), 'RECOVERY_CONTRACT_PREFLIGHT_REQUIRED'); }
