...
export function report(root: string) {
    ...
    try {
        ...
        const observations = c.schedule.map((item: any) => {
            ...
            return { ...item, ...statusFrom(store, c, item.runId), checks, deterministicAccepted: Object.values(checks).every(Boolean) && ['completed', 'blocked', 'rejected'].includes(run.phase), semanticAccepted: semanticReview?.accepted ?? null, semanticReview, actualBilledMinor, humanWorkSeconds: semanticReview?.reviewSeconds != null && semanticReview?.correctionSeconds != null && (!positive || approval.some(e => e.source === 'human')) ? semanticReview.reviewSeconds + semanticReview.correctionSeconds + approval.reduce((n, e) => n + (e.measuredSeconds ?? 0), 0) : null, semanticReviewProvenance: c.mode === 'mock' ? 'deterministic mock output; semantic quality unmeasured' : 'human review required', decision: run.decision, artifact: observed?.artifact ?? null, inspection, prepublicationReview: review ?? null, teamProposal: store.get('team-proposal', item.runId), failures, seeded: ep.seeding, independentHumanCorrectionSeconds: null, approvalReviewSeconds: approval.some(e => e.source === 'human') ? approval.reduce((n, e) => n + (e.measuredSeconds ?? 0), 0) : null, attemptIds: part.map(a => a.id), latencyMs: part.reduce((n, a) => n + (a.observation?.latencyMs ?? 0), 0), inputTokens: part.reduce((n, a) => n + (a.observation?.inputTokens ?? 0), 0), outputTokens: part.reduce((n, a) => n + (a.observation?.outputTokens ?? 0), 0), provisionalMinor: part.reduce((n, a) => n + (a.cost?.status === 'provisional' ? a.cost.money.minorUnits : 0), 0), effectCount: observed?.effectCount ?? 0, simulatedBusinessSpentMinor: action?.actualCost?.status === 'known' ? action.actualCost.money.minorUnits : 0 };
        });
        const paired = episodes.map(ep => { const a = observations.find((x: any) => x.episode === ep.id && x.configuration === 'single'), b = observations.find((x: any) => x.episode === ep.id && x.configuration === 'team'); return { episode: ep.id, lineage: ep.lineage, singleAccepted: a.deterministicAccepted, teamAccepted: b.deterministicAccepted, mechanicalDifference: Number(b.deterministicAccepted) - Number(a.deterministicAccepted), semanticDifference: null }; });
        const complete = observations.every((x: any) => x.deterministicAccepted);
        const result = { version: c.version, configHash: hash(c), provenance: c.provenance, recordedAt: new Date().toISOString(), status: complete ? (c.mode === 'mock' ? 'offline_mechanics_complete' : 'mechanics_complete_review_required') : 'incomplete_or_failed', observations, paired, allAttempts: attempts, accounting: { currency: 'USD', actualProviderCalls: c.mode === 'mock' ? 0 : attempts.filter(x => x.inferenceDispatchIntent).length, actualTokenCounts: c.mode === 'mock' ? 0 : attempts.filter(x => x.countDispatchIntent).length, mockAttempts: c.mode === 'mock' ? attempts.length : 0, supportingCountBufferMinor: c.limits.overheadReserve?.minor ?? 0, reservedMinor: attempts.reduce((n, x) => n + x.reservation, 0), provisionalMinor: attempts.reduce((n, x) => n + (x.cost?.status === 'provisional' ? x.cost.money.minorUnits : 0), 0), authoritativeSettledMinor: attempts.reduce((n, x) => n + (x.invoice?.minorUnits ?? 0), 0), actualBilledMinor: c.mode === 'mock' ? 0 : null, mode: c.mode, historicalMission028: { retainedMinor: 871, provisionalMinor: 124, transferredMinor: 0 } }, analysis: analyze(observations, c.mode), inference: { decision: c.mode === 'mock' ? 'No AI comparison inference; offline mechanics only' : 'Await independent semantic reviews and paired analysis', independentUnits: 6, relatedCases: true, reliabilityClaim: false, qualification: 'experimental_non_production', humanCorrectionEffort: null }, capabilityRecords: ['single', 'team'].map(configuration => ({ configuration, version: c.version, taskScope: 'Evidence-to-approved synthetic internal playbook', contextRequirements: 'Versioned business brief, current costs/policy, exact proposal, signed receipt/readback', tools: ['lab.evidence', 'lab.publish', 'lab.readback'], authority: 'Exact human approval; no customer effects', evidencePopulation: 'Six developer-visible synthetic episodes; related cost/policy families', provenance: c.mode, observedAICompetence: null, semanticQuality: null, humanCorrectionEffort: null, qualification: 'experimental_non_production', knownLimitations: ['Offline mock outcomes are engineering evidence only', 'No protected final evaluation', 'No team optimality inference', 'Provider revision alias not proven immutable'], roleEvidence: 'Mission028 limited short-response evidence does not establish workflow competence' })) };
