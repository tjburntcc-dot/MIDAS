...
export class PortfolioEngine {
 ...
 private context(task:Task){
  ...
  const explicitHandoff=Boolean((task.inputs as any)?.sourceBindings||workspace?.inputs?.enforceHandoff||workspace?.inputs?.profile==='quote-to-job-v2');const currentFiles=(workspace?.files??[]).filter((f:any)=>workspace?.kind!=='service'||f.path==='brief.json');const currentFileBytes=Buffer.byteLength(JSON.stringify(currentFiles));const admittedSourceBytes=workspace?.kind==='service'?Buffer.byteLength(currentFiles[0]?.content??''):currentFileBytes;requireThat(!explicitHandoff||admittedSourceBytes<=(workspace?.kind==='software'?24000:workspace?.inputs?.operatingProfile==='operating-packet-v1'?9000:18000),'CURRENT_SOURCE_HANDOFF_TOO_LARGE');
 ...
 private async call(lease:TaskLease,planning=false){
  ...
  if(saved)result=this.settleSaved('portfolio-model-result',key,identity);
   ...
   try{result=await this.model.run(call);}catch(e){if((e as any)?.simulatedCrash)throw e;const error=e as any,knownLocal=error?.code==='PORTFOLIO_INITIAL_REQUEST_CHANGED'&&['expected','actual'].every(k=>/^[a-f0-9]{64}$/.test(error[k]?.bodyHash)&&/^[a-f0-9]{64}$/.test(error[k]?.schemaHash));this.portfolio.finishStep(t.id,lease.token,stepId,{uncertain:!knownLocal,error:code(e),...(knownLocal?{outputRef:{kind:'local_authority_rejection',providerAdmission:false,expected:error.expected,actual:error.actual}}:{})});throw e;}
