...
export const CAPABILITY_PROFILES:Record<string,{description:string;requiredTools:string[]}>={
 ...
 'quality.review':{description:'Review and replace the full authoritative source-grounded packet, with the original artifact bound in task inputs. Corrections must preserve required source coverage and propagate to downstream work.',requiredTools:deliveryTools},
...
export class PortfolioEngine {
 ...
 private context(task:Task){
  ...
  const dependencies=task.dependsOn.map(id=>{const parent=this.portfolio.getTask(id);return {id,objective:parent.objective??parent.title,status:parent.status,outputCurrent:parent.outputCurrent,summary:parent.result?.summary??null,artifacts:parent.outputArtifacts.map(binding=>{
   ...
   if(planning&&a.taskId&&a.downloadUrl){const upstream=this.tools.load(task.ventureId,a.taskId);if(upstream.kind==='service'){
  ...
  let previewBudget=Math.max(0,Math.min(16000,42000-Buffer.byteLength(JSON.stringify(observations))-reportBytes));
 ...
 private async plan(lease:TaskLease){
  const t=lease.task,result=await this.call(lease,true),p=result.output as PlanningProposal;
  ...
  this.store.record(workerScope(t.ventureId,t.id),'executive-proposal-'+hash(t.id).slice(0,20),'portfolio.executive_proposal',{proposal:p,sourceSnapshot:this.evidence.list(t.ventureId),provenance:this.model.kind,semanticTruth:'proposal_not_established_causality'});
