...
function executionStrip(items, decisions = []) {
  ...
  return `<section class="execution-strip" aria-label="Execution status"><article><span>Running</span><strong>${status.running}</strong><small>Active assignments</small></article><article><span>Task records complete</span><strong>${status.completed}</strong><small>Saved completion state; separate from provider, customer, or revenue evidence</small></article><article class="${status.blocked ? 'attention' : ''}"><span>Blocked or stale</span><strong>${status.blocked}</strong><small>${status.blocked ? 'Needs a resolved constraint' : 'No saved constraint'}</small></article><article class="${status.awaitingOwner ? 'awaiting' : ''}"><span>Awaiting owner</span><strong>${status.awaitingOwner}</strong><small>Decision or signed grant</small></article></section>`;
}
function terminalDecisionReason(item) {
  const provider = item?.providerExecution;
  if (['terminal_failed','terminal_cancelled'].includes(provider?.completionState)) return `Provider response is terminal ${provider.status}. No accepted task result; missing usage is accounting uncertainty, not unknown completion.`;
  if (provider?.completionState !== 'terminal_incomplete' || item.status !== 'needs_reconciliation') return item?.reason;
  const incompleteCount = Number.isSafeInteger(provider.terminalIncompleteAttempts) && provider.terminalIncompleteAttempts > 0 ? provider.terminalIncompleteAttempts : null;
  const priorUnknown = Boolean(item.inputs?.continuationReplacement?.preservesUncertainOutcome);
  return `No accepted build decision; ${incompleteCount === null ? 'the' : incompleteCount} provider outcome${incompleteCount === 1 ? '' : 's'} ${incompleteCount === 1 ? 'is' : 'are'} known incomplete.${priorUnknown ? ' The separate prior R2 outcome remains unknown.' : ''}`;
}
function modelAccountingPanel(accounting = state.data?.modelAccounting) {
  ...
  const summary = accounting.status === 'available'
    ? accounting.historicalOnly ? 'A preserved parent signed ledger is verified against this copied database. It remains historical evidence only; R4 has no active model grant or execution authority.' : 'A signed model ledger is recorded. It is read-only and separate from venture economics; its grant mode does not establish that a provider call, charge, or customer result occurred.'
  ...
  const status = accounting.status === 'available' ? accounting.historicalOnly ? 'Historical only · no R4 authority' : `${accounting.expired ? 'Expired' : accounting.revoked ? 'Revoked' : accounting.halted ? 'Halted' : 'Available'} · read-only` : 'Unfunded · read-only';
  const currentLimit = Number.isFinite(accounting.callLimit) ? `${value(accounting.callsUsed)}/${value(accounting.callLimit)}` : value(accounting.callsUsed);
  const continuedLimit = Number.isFinite(accounting.combinedCallLimit) ? `${value(accounting.combinedCallsUsed)}/${value(accounting.combinedCallLimit)}` : null;
  const limit = continuedLimit || currentLimit;
  ...
  const concise = `${accounting.status === 'unfunded' ? 'Unfunded' : 'Signed ledger'} · ${limit} admissions${continuedLimit ? ' across continuation' : ''} · ${amount(accounting.retainedMinor)} retained`;
  const settlementDetail = accounting.settledBillingStatus === 'recorded' ? `${value(accounting.settlementCount)} recorded settlement${accounting.settlementCount === 1 ? '' : 's'}`
    : accounting.settledBillingStatus === 'partially_recorded' ? `${value(accounting.settlementCount)} settlement${accounting.settlementCount === 1 ? '' : 's'} recorded; other admitted dispatches have no settlement evidence`
      : accounting.settledBillingStatus === 'not_recorded' ? 'No settlement evidence recorded for admitted dispatches'
        : accounting.settledBillingStatus === 'unknown' ? 'Billing is unknown for at least one admitted dispatch'
          : 'No billable dispatch is recorded';
  const settlementValue = accounting.settlementCount ? amount(accounting.settledMinor) : accounting.settledBillingStatus === 'unknown' ? 'Unknown' : 'None recorded';
  const evidence = accounting.ledgerEvidence === 'signed_current_grant' ? 'Current signed ledger' : accounting.ledgerEvidence === 'preserved_historical_signed_ledger' ? 'Preserved parent signed ledger' : accounting.ledgerEvidence === 'historical_fixture' ? 'Historical fixture ledger' : accounting.ledgerEvidence === 'historical_provider_evidence' ? 'Historical provider receipts' : accounting.ledgerEvidence === 'mixed_historical' ? 'Mixed historical ledger' : accounting.ledgerEvidence === 'historical_ledger' ? 'Historical ledger' : 'Ledger evidence not classified';
  const continuation = continuedLimit ? metric('Current grant admissions', currentLimit, 'Current stage within the exact continuation; the combined figure includes retained parent admissions') : '';
  const continuationEstimate = Number.isFinite(accounting.combinedProvisionalMinor) ? metric('Continuation estimate subtotal', amount(accounting.combinedProvisionalMinor), 'Recorded provisional estimates across the exact carried history; not a settlement') : '';
  return `<details class="model-accounting"><summary><span><strong>${escape(title)}</strong><small>${escape(concise)}</small></span>${badge(status, accounting.status === 'available' ? 'prepared' : 'unknown')}</summary><div class="model-accounting-body"><p>${summary}</p><div class="model-accounting-grid">${metric('Ledger evidence', evidence, 'Signed or retained record only; it does not prove a live provider outcome')}${metric('Inference admissions', limit, continuedLimit ? 'Exact continuation total / combined cap' : 'Admitted model calls / configured cap')}${continuation}${metric('Inference dispatches', value(accounting.inferenceDispatches), 'Recorded provider-bound dispatch intents in this signed grant')}${metric('Count requests', value(accounting.countRequests), 'Recorded separate count-endpoint intents in this signed grant')}${metric('Provisional estimate', amount(accounting.provisionalMinor), provisionalDetail)}${continuationEstimate}${metric('Authoritative settlements', settlementValue, settlementDetail)}${metric('Signed-ledger retained exposure', amount(accounting.retainedMinor), `Known reservation exposure, including ${amount(accounting.countBufferMinor)} count buffer; not a provider charge`) }${metric('Remaining cap', amount(accounting.remainingMinor), accounting.status === 'unfunded' ? 'No funded cap is available' : 'Read-only remaining allowance')}</div><p class="model-accounting-note">Execution authority: ${accounting.executionAuthority ? 'recorded by the signed grant' : 'not present'}. ${accounting.reason ? escape(accounting.reason) : ''}</p></div></details>`;
}
...
function portfolioView() {
  ...
  return `<div class="page-heading"><div><p class="eyebrow">THE BIG PICTURE</p><h1>Your portfolio</h1><p>Businesses, useful work and the decisions that move them forward.</p></div><div class="heading-aside heading-tools"><span class="badge provenance">Local workspace</span>${state.data.operatingPacket ? button('Owner operating packet', 'operating-packet', {}, 'small') : ''}</div></div>
    ...
    <div class="metrics">${metric('Ventures', data.ventures.length, 'Research, prototypes and operating businesses')}${metric('Products ready', products.length, products.length ? 'Open and inspect the recorded outputs' : 'Outputs appear after work produces them')}${metric('Business operating costs', money(economics.cost, economics.currency), economics.cost === null ? 'No business-cost record; model ledger above is separate' : 'Recorded business operating cost')}${metric('Business action reservations', money(economics.reserved, economics.currency), economics.reserved === null ? 'No business action reservation; model ledger above is separate' : 'Held business action exposure, separate from cost')}</div>
    ...
    <div class="two-columns">${panel('Latest changes', activityList(), navigation('View work →', 'work'))}${panel('What needs your attention', decisions.length ? decisions.slice(0, 3).map(item => `<article class="decision-preview"><h3>${escape(item.title)}</h3><p>${escape(excerpt(item.reason || item.summary || item.recommendation, 180))}</p>${navigation('Review decision →', 'decisions')}</article>`).join('') : issues.length ? issues.slice(0, 3).map(item => `<article class="decision-preview"><h3>${escape(item.title)}</h3><p>${escape(terminalDecisionReason(item) || 'Work requires attention before it can continue.')}</p>${navigation('Inspect assignment →', 'work', item.ventureId)}</article>`).join('') : empty('No owner decision waiting', 'MIDAS can continue eligible local work within its configured boundaries.'))}</div>`;
}
...
function taskCard(item) {
  ...
  const provider = item.providerExecution;
  const terminalReason = terminalDecisionReason(item);
  const providerSummary = provider?.completionState === 'terminal_completed' ? 'Provider recorded a terminal completed response. Task acceptance remains separate.'
    : ['terminal_incomplete','terminal_failed','terminal_cancelled'].includes(provider?.completionState) ? `Provider recorded a terminal ${provider.status || 'incomplete'} response. No completed provider result is claimed.`
      : provider?.completionState === 'in_progress' ? 'Provider response identity is saved. Recovery reads the same response; it does not start another inference.'
        : 'No provider response identity is saved. Provider completion is unknown; automatic resubmission is blocked.';
  return `<article class="task-card" data-task-card="${escape(item.id)}"><div><div class="task-eyebrow">${badge(lane(item))}${badge(item.status)}${!state.ventureId ? `<span>${escape(venture(item.ventureId)?.name || 'Venture')}</span>` : ''}</div><h3>${escape(item.title)}</h3><p>${escape(item.objective || item.description || '')}</p>${terminalReason && (!['completed', 'queued'].includes(item.status) || item.outputCurrent === false || item.runnable === false) ? `<p class="task-reason">${escape(terminalReason)}</p>` : ''}${provider ? `<p class="task-reason">${escape(providerSummary)} Read requests: ${escape(provider.retrievalRequests)}.</p><details><summary>Long-running response and recovery</summary><p>Provider status: ${escape(provider.status)}. Completion deadline: ${escape(provider.completionDeadlineAt || 'Not dispatched')}. Latest permitted retrieval: ${escape(provider.resumeUntil || 'Not dispatched')}.</p><p>Response identity: ${escape(provider.responseId || 'Unavailable')}. Provider completion, task completion, billing, and customer acceptance are recorded separately.</p></details>` : ''}${item.result?.summary ? `<p>${escape(item.result.summary)}</p>` : ''}${dependencies.length || acceptance.length || checks.length ? `<details><summary>Completion criteria, dependencies and checks</summary>${acceptance.length ? `<ul>${acceptance.map(rule => `<li>${escape(typeof rule === 'string' ? rule : rule.description || rule.label)}</li>`).join('')}</ul>` : ''}${dependencies.length ? `<p>Depends on: ${dependencies.map(dependency => `${escape(dependency.title)} (${escape(stateLabels[dependency.status] || words(dependency.status))})`).join('; ')}.</p>` : ''}${checks.length ? renderChecks(checks) : ''}</details>` : ''}<div class="task-footer"><span>Worker: ${escape(workerName(item))}</span>${execution ? `<span>${escape(execution)}</span>` : ''}${item.updatedAt ? `<span>Updated ${escape(date(item.updatedAt))}</span>` : ''}</div></div><div class="task-actions">${actions.map(action => button(action === 'run' && /offline|fixture/i.test(execution) ? 'Run fixture' : words(action), action, { task: item.id, venture: item.ventureId }, `small ${action === 'run' || action === 'resume' ? 'primary' : action === 'cancel' ? 'danger' : ''}`)).join('')}${array(item.outputArtifacts).length || array(item.result?.artifacts).length ? navigation('Inspect output', 'products', item.ventureId, 'button small') : ''}</div></article>`;
}
