...
function portfolioView() {
  ...
  return `<div class="page-heading"><div><p class="eyebrow">THE BIG PICTURE</p><h1>Your portfolio</h1><p>Businesses, useful work and the decisions that move them forward.</p></div><div class="heading-aside heading-tools"><span class="badge provenance">Local workspace</span>${state.data.operatingPacket ? button('Owner operating packet', 'operating-packet', {}, 'small') : ''}</div></div>
    ...
    <div class="metrics">${metric('Ventures', data.ventures.length, 'Research, prototypes and operating businesses')}${metric('Open work', runnableTasks(data.tasks).length, `${active.length} running · ${issues.length} need attention`)}${metric('Products & deliveries', products.length, products.length ? 'Inspect outputs and their origin below' : 'Outputs appear after work produces them')}${metric('Owner decisions', decisions.length, decisions.length ? 'Prepared choices across the portfolio' : 'No decision waiting on you')}</div>
...
function ventureOverview() {
  ...
  return `<div class="two-columns equal">${panel('The business thesis', `<div class="fact-grid"><div><span class="fact-label">Customer</span><div class="fact-value">${escape(item.customer || item.buyer || 'Customer definition not recorded.')}</div></div><div><span class="fact-label">Current commitment</span><div class="fact-value">${escape(words(item.commitment || item.stage || 'research'))}</div></div></div><div class="insight"><strong>Why pursue this?</strong><p>${escape(why || 'The opportunity remains a hypothesis. Research should establish the customer job, substitutes and access route.')}</p></div><div class="insight"><strong>What must be resolved</strong><p>${escape(bottleneck || item.uncertainty || 'Customer demand and repeatable economics have not been established.')}</p></div>${item.goal ? `<details class="technical"><summary>Full venture objective</summary><p class="artifact-content">${escape(item.goal)}</p></details>` : ''}`)}
    ...
    ${sectionHeading('Products & deliveries', 'Inspect the work and what produced it.', navigation('View all →', 'products', item.id))}${outputs.length ? `<div class="product-grid">${outputs.slice(-2).reverse().map(productCard).join('')}</div>` : empty('No product or delivery yet', 'Completed output will appear here with its origin, version and inspection links.')}
...
function productsView() {
  ...
  return `${!state.ventureId ? `<div class="page-heading"><div><p class="eyebrow">TANGIBLE OUTPUT</p><h1>Products & deliveries</h1><p>Open working previews, inspect deliverables, and see how each output was produced.</p></div></div>` : ''}${artifacts.length ? `<div class="product-grid">${artifacts.map(productCard).join('')}</div>` : empty('No outputs to inspect yet', 'Products and deliverables appear after a task records an artifact. Preparation and execution remain separately identified.')}`;
}
...
function productCard(item) {
  ...
  return `<article class="product-card"><div class="product-visual"><span class="product-icon" aria-hidden="true">${isSoftware ? '▱' : /report|brief|document|service/i.test(kind) ? '≡' : '◈'}</span><span class="product-kind">${escape(words(kind))}</span></div><div class="product-body"><div class="card-meta">${badge(`Version ${item.version ?? 1}`, 'provenance')}${badge(authored(item.provenance || item.authorship), 'provenance')}${item.status ? badge(item.status) : ''}${sourceTask?.outputCurrent === false ? badge('Inputs changed', 'stale') : ''}</div><h3>${escape(item.title || 'Untitled output')}</h3>${!state.ventureId ? `<p>${escape(venture(item.ventureId)?.name || '')}</p>` : ''}<p>${escape(item.summary || item.description || (preview ? 'Open the local preview to try this output.' : 'Inspect the saved output and its execution evidence.'))}</p>${checks.length ? renderChecks(checks) : ''}<div class="actions">${preview ? `<a class="button primary" href="${escape(preview)}" target="_blank" rel="noopener noreferrer">${isSoftware ? 'Open product ↗' : 'Open preview ↗'}</a>` : ''}${button('Inspect', 'inspect-artifact', { artifact: item.id }, 'small')}${download ? `<a class="button small" href="${escape(download)}" download>Download</a>` : ''}${item.revisable === true ? button('Request revision', 'open-revision', { artifact: item.id }, 'small') : ''}</div>${artifactOriginNote(item) ? `<p class="provenance-note">${escape(artifactOriginNote(item))}</p>` : ''}</div></article>`;
}
