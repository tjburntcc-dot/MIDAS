...
export class Portfolio {
    ...
    private claimInside(taskId: string, workerId: string, options: { ownerId?: string; leaseMs?: number } = {}): TaskLease | null {
        const t = this.task(taskId), w = this.store.get(kinds.worker, workerId); requireThat(w, 'WORKER_NOT_FOUND'); if (!w.available || !this.ready(t)) return null;
        ...
        if (t.attempts >= t.maxAttempts) { this.put(kinds.task, { ...t, status: 'blocked', reason: 'Attempt limit reached', nextAction: 'Inspect failure evidence and revise the plan', updatedAt: this.at() }); return null; }
        const leaseMs = options.leaseMs ?? this.leaseMs; integer(leaseMs, 1, 86_400_000);
        const lease: Lease = { token: randomUUID(), ownerId: options.ownerId ?? String(process.pid), workerId, generation: t.attempts + 1, heartbeatAt: this.at(), expiresAt: new Date(this.now() + leaseMs).toISOString() };
        ...
        const task = this.put(kinds.task, { ...t, status: 'running', reason: 'Worker is executing the persisted assignment', nextAction: 'Inspect progress or pause work', attempts: t.attempts + 1, workerId, lease, stopRequested: null, inputArtifacts: bindings, progressAt: this.at(), updatedAt: this.at() });
        this.event(t.ventureId, 'task_claimed', { taskId, workerId, generation: lease.generation }); return { task, token: lease.token, lease };
    }
    ...
    recover(options: { ownerAlive?: (ownerId: string) => boolean } = {}) { return this.store.transaction(() => { const recovered = []; for (const t of this.rows(kinds.task).filter(t => t.lease)) { const lease = t.lease, stalled = this.now() - Date.parse(t.progressAt ?? t.updatedAt) > this.stallMs; if (!stalled && Date.parse(lease.expiresAt) > this.now() && (!options.ownerAlive || options.ownerAlive(lease.ownerId))) continue; const unknown = this.unresolved(t.id).length > 0, cancelled = t.stopRequested === 'cancel' || this.getVenture(t.ventureId).status === 'cancelled', paused = t.stopRequested === 'pause' || this.getVenture(t.ventureId).status === 'paused'; const status = unknown ? 'needs_reconciliation' : cancelled ? 'cancelled' : paused ? 'paused' : t.invalidatedAt ? 'stale' : stalled || t.attempts >= t.maxAttempts ? 'blocked' : 'queued'; const next = this.put(kinds.task, { ...t, lease: null, status, reason: unknown ? 'Worker stopped with an admitted step whose outcome is uncertain' : stalled ? 'Worker heartbeat continued without task progress; diagnosis required' : 'Worker lease ended; durable task checkpoint retained', nextAction: unknown ? 'Reconcile preserved attempt; do not dispatch again' : status === 'queued' ? 'Resume from the persisted checkpoint' : 'Inspect stopping reason', updatedAt: this.at() }); recovered.push(next); this.event(t.ventureId, 'task_recovered', { taskId: t.id, status, priorGeneration: lease.generation, stalled }); } return recovered; }); }
