...
export type WorkerDecision = {action:'tool'|'complete'|'blocked';reason:string;toolCall:null|{name:typeof TOOL_NAMES[number];arguments:{path:string|null;content:string|null;expectedHash:string|null;query:string|null;url:string|null}}};
export function validateWorker(out:unknown):asserts out is WorkerDecision {
 ...
 if(x.toolCall.name==='research.read')requireThat(/^\d+$/.test(a.query??''),'SOURCE_OFFSET_INVALID');
...
export function makeWorkerRequest(x:{ventureId:string;taskId:string;attemptId:string;context:unknown;tools:string[];procedure?:string;model?:string;maxMinor?:number}):ModelRequest{
 return {scope:workerScope(x.ventureId,x.taskId),requestId:x.attemptId,task:'operate',role:{id:'business-worker',version:WORKER_VERSION,procedure:x.procedure??WORKER_PROCEDURE,competencies:['evidence-grounded-work','artifact-correction'],tools:x.tools,predecessor:null,model:x.model??'gpt-6-astra',qualification:'experimental_unqualified'},context:x.context,limits:{maxCost:{currency:'USD',minorUnits:x.maxMinor??123},maxAttempts:1,maxHumanMinutes:0},tools:x.tools.map(name=>({name,version:'portfolio-tool-v1'}))};
}
