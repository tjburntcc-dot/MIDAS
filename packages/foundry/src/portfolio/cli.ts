...
else if(command==='run'){await engine.recover();if(live?.authorization.route.background)await engine.resumeAdmittedResponses();const task=flag('--task','');const result=task?await engine.runTask(task):await engine.drain();prepareCommercialPackets(portfolio);json({result,accounting:live?.totals()??{providerRequests:0,localComputeCost:null}});store.close();}
...
else if(command==='resume-response'){requireThat(live?.authorization.route.background,'BACKGROUND_GRANT_REQUIRED');await engine.recover();json({recovery:await engine.resumeResponse(flag('--task','')),accounting:live.totals()});store.close();}
