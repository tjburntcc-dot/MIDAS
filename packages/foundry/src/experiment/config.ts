...
export function implementationHash() { const here = dirname(fileURLToPath(import.meta.url)); const names = ['analysis', 'cli', 'config', 'custodian', 'ledger', 'provider', 'rehearsal', 'task', 'workflow']; return hash([...names.map(n => readFileSync(join(here, n + '.ts'), 'utf8').replace(/\r\n/g, '\n')), ...['contracts', 'state', 'model-port'].map(n => readFileSync(join(here, '..', n + '.ts'), 'utf8').replace(/\r\n/g, '\n'))]); }
