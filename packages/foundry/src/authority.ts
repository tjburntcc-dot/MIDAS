...
export class Authority {
    ...
    initializeBusiness(s: Scope, cap: Money) {
        ...
        const existing = this.store.get('business', key);
        if (existing) {
            requireThat(existing.policyVersion === s.dataPolicyVersion, 'POLICY_MISMATCH');
            requireThat(existing.cap.currency === cap.currency, 'CURRENCY_MISMATCH');
            return;
        }
    ...
    approve(s: Scope, principal: Principal, p: Proposal, options: any = {}) {
        ...
        proposal(p);
        requireThat(scopeKey(s) === scopeKey(p.scope), 'SCOPE_DENIED');
        ...
        const maxCost = money(options.maxCost ?? p.estimatedCost);
        const maxActions = options.maxActions ?? 1;
        safeInteger(maxActions, 1);
        const reviewMinutes = options.reviewMinutes ?? 2;
        safeInteger(reviewMinutes);
        return this.store.transaction(() => {
            this.bindRun(s, p);
            const task = this.store.get('run', scopeKey(s)).plan.tasks.find((t: any) => t.id === p.taskId);
            requireThat(reviewMinutes <= task.maxHumanMinutes, 'HUMAN_REVIEW_LIMIT');
            const b = this.business(s);
            requireThat(b.policyVersion === p.policyVersion && b.stateVersion === p.businessVersion, 'STALE_APPROVAL');
            const key = scopeKey(s) + '/' + p.id;
            const old = this.store.get('grant', key);
            if (old) {
                requireThat(old.proposalHash === hash(p) && !old.revoked, 'APPROVAL_ALREADY_EXISTS');
                return old;
            }
    ...
    reserve(p: Proposal, now = Date.now()): {
        execute: boolean;
        action: any;
    } {
        proposal(p);
        ...
        try {
            return this.store.transaction(() => {
                ...
                if (existing) {
                    requireThat(existing.proposalHash === hash(p), 'IDEMPOTENCY_CONFLICT');
                    return { execute: false, action: existing };
                }
                this.bindRun(s, p);
                const run = this.store.get('run', scopeKey(s));
                requireThat(!run?.cancelled, 'RUN_CANCELLED');
                const g = this.store.get('grant', key);
                const b = this.business(s);
                requireThat(g, 'APPROVAL_REQUIRED');
                requireThat(!g.revoked, 'APPROVAL_REVOKED');
                ...
                const task = run.plan.tasks.find((t: any) => t.id === p.taskId);
                requireThat(task.deadline === undefined || Date.parse(task.deadline) > now, 'TASK_DEADLINE_EXPIRED');
                ...
                safeInteger(b.spent + b.reserved + reserve);
                safeInteger(g.spent + g.reserved + reserve);
            ...
            this.store.transaction(() => this.store.event(s, 'action_denied', { id: p.id, code: (error as any).code ?? 'STATE_ERROR', retried: false }));
            throw error;
        }
    }
    settle(p: Proposal, observation: Observation, verifyObservation?: (p: Proposal, o: Observation) => boolean) {
        object(observation, ['status'], ['externalReceiptId', 'payloadHash', 'artifact', 'ledger', 'actualCost', 'effectCount', 'proof']);
        requireThat(['confirmed', 'absent', 'unknown', 'failed'].includes(observation.status), 'INVALID_OBSERVATION');
        if (observation.actualCost)
            cost(observation.actualCost);
        return this.store.transaction(() => {
            const s = p.scope, key = scopeKey(s) + '/' + p.id;
            const a = this.store.get('action', key);
            requireThat(a && a.proposalHash === hash(p), 'ACTION_NOT_FOUND');
            if (a.status === 'confirmed' || a.status === 'failed')
                return a;
            ...
            if (confirmed)
                requireThat(observation.payloadHash === p.payloadHash && typeof observation.externalReceiptId === 'string' && observation.externalReceiptId.length > 0, 'INVALID_RECEIPT');
            const known = amount?.status === 'known';
            if (known) {
                requireThat(amount.money!.currency === b.cap.currency, 'CURRENCY_MISMATCH');
                requireThat(amount.money!.minorUnits <= a.reservation, 'COST_EXCEEDS_RESERVATION');
            }
            // Absence is safe to release only when the adapter's reconciliation is authoritative.
            const negativeProven = observation.status === 'absent' || observation.status === 'failed'
                ? verifyObservation?.(p, observation) === true && observation.effectCount === 0 : true;
            const resolved = known && observation.status !== 'unknown' && negativeProven;
            ...
            if (resolved)
                this.store.record(s, 'receipt-' + p.id, 'ActionReceipt', { proposalHash: hash(p), status: next.status, externalReceiptId: next.externalReceiptId, actualCost: next.actualCost, requestDigest: p.payloadHash });
            return next;
        });
    }
    action(p: Proposal) { return this.store.get('action', scopeKey(p.scope) + '/' + p.id); }
    private bindRun(s: Scope, p: Proposal) {
        const run = this.store.get('run', scopeKey(s));
        requireThat(run && ['waiting_approval', 'reconciling'].includes(run.phase) && !run.cancelled, 'RUN_NOT_ACTIONABLE');
        requireThat(run.proposal && hash(run.proposal) === hash(p), 'RUN_PROPOSAL_MISMATCH');
        requireThat(p.roleVersion === run.roles.operator.version && run.roles.operator.tools.includes(p.toolId), 'PINNED_ROLE_MISMATCH');
        requireThat(run.plan.tasks.some((task: any) => task.id === p.taskId && task.roleArtifact.version === p.roleVersion && task.permittedToolIds.includes(p.toolId)), 'TASK_AUTHORITY_MISMATCH');
    }
}
